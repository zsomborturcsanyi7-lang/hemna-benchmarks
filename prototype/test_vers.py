...
class LM(torch.nn.Module):
    def __init__(self, V, dim=1024, layers=24, heads=16, ffn=3072):
        super().__init__()
...
prompts = ["Vers az elrol:", "Vers a szerelemrol:", "Vers a gepi tanulasrol:"]
out_file = r'C:\NeuraNode\hemna_bench\versek.txt'
with codecs.open(out_file, 'w', 'utf-8') as f:
    for prompt in prompts:
        ids = torch.tensor(sp.EncodeAsIds(prompt))
        text = model.generate(ids, n=150, t=0.8, k=30)
        f.write(f"\n--- {prompt} ---\n{text[:400]}\n")
