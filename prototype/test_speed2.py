"""Teszt: hany mp az 1 forward+backward+optimizer step?"""
import torch, time
...
for step in range(5):
    ...
    opt.zero_grad()
    t2 = time.time()
    print(f'Step {step}: fwd+bwd={t1-t0:.3f}s opt={t2-t1:.3f}s tot={t2-t0:.3f}s mem={torch.cuda.memory_allocated()/1024**3:.2f}GB', flush=True)
